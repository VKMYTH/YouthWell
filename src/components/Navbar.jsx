...
const Navbar = () => {
  ...
  const handleScroll = () => {
    const sections = ['home', 'services', 'about', 'chatbot', 'testimonial'];
  ...
  const navLinks = (
    <ul className="font-medium flex flex-col md:flex-row lg:space-x-8 sm:space-x-4 space-y-2 md:space-y-0 p-4 md:p-0">
      ...
      <li>
        <motion.a
          href="#chatbot"
          className={`text-white ${activeSection === 'chatbot' ? 'isactive' : ''}`}
  ...
  return (
    <header className="bg-heroBg text-white py-6 px-4 fixed top-0 left-0 right-0 z-10">
      ...
      <div className="text-white text-lg font-semibold flex items-center h-12">
        <a href="/">
          <img 
            src="/logo.png" 
            alt="YouthWell Logo" 
